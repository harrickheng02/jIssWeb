...
- **只做审查与报告**：不执行修改、不运行会**改动业务代码或 OpenSpec 制品**的命令（可只读查看文件、diff）。**例外**：为对照规范面，允许在仓库根执行 **`npm run graph:route`**，其唯一副作用是覆盖 **`scripts/gitee-sync/.last-route.txt`**（已 gitignore，非源码）。
...
| 做法 | 说明 |
|------|------|
| **PR / CI 门禁** | 优先在流水线或 PR 模板中固定 `dotnet test`、`npm run test`（或解决方案级测试），与验收对齐，避免依赖单次人工审查。 |
| **tasks.md** | 若某条验收已由自动化测试覆盖，可在任务下 **简要记录** 项目路径与命令（例如 `backend/tests/...`、`dotnet test ...`），便于审查交叉核对。 |
| **本技能默认** | **不主动执行**测试命令，以免拉长会话、依赖本机环境；以读代码、读 spec、读 tasks 为主。 |
| **用户明确要求** | 若在同一会话中要求「跑测试再审」，可执行只读验证类命令并将结果写入报告「测试与验证」小节。 |
...
- `tasks.md` 中勾选项：实现是否在代码中有对应体现（或明确延期/不在范围）；若条目中写了测试/命令记录，是否与当前分支一致。
- `specs/*.md` 中的需求/场景：是否有实现或显式缺口。
- 变更说明与 `design.md` 中的架构决定是否被遵守。

### 图谱路由对照（repo-knowledge-router）

- 在仓库根执行：  
  `npm run graph:route -- -- "<change 名、需求短语或关联 issue 标题>"`  
  结果写入 **`scripts/gitee-sync/.last-route.txt`**；审查时优先 **`@` 该文件**（或终端 `path` 列）作为「应读规范面」。报告中写明：是否覆盖本次 diff 所涉能力；遗漏则标 **中**/**低** 并建议补 `pm-plan` / OpenSpec 引用。
...
## 测试与验证（可选小节）
- 本会话是否执行自动化测试；若未执行，写明「仅静态审查」。
- 若已执行：命令摘要、通过/失败、与 `tasks.md` 验收是否一致。
- 若适用：指出缺失的 CI 门禁或 `tasks.md` 中未记录的验证方式。
...
- 不要将「审查通过」等同于「测试已在 CI 通过」；二者需在报告或流程中区分说明。
