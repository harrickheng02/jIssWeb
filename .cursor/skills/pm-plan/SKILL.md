...
name: pm-plan
description: >-
  分析、对照与维护 pm-plan.yaml（路径由 git remote origin 在 scripts/github-sync 与 scripts/gitee-sync 间解析）；在仓库根代执行 npm（pm:pull、graph:refresh、pm:publish 等）。用户提到 pm 规划、里程碑、Issue、pm-plan、同步 GitHub/Gitee、归档后回写规划时启用。
license: MIT
metadata:
  author: project
  version: "1.4.1"
...
- 缺 `scripts/github-sync` 或 `repo-knowledge-router` 依赖时执行 **`npm run pm:ci`**。对话与 Git 中勿出现 `GITEE_*`、`GITHUB_TOKEN`、`.env` 内容。
...
| 改完规划后推 GitHub/Gitee（先 refresh 再 push 门禁） | `npm run pm:publish` |
...
- **`scripts/github-sync/.env`**（或 `scripts/gitee-sync/.env`）勿写入对话或 Git。
...
- `scripts/github-sync/pm-plan.yaml` 或 `scripts/gitee-sync/pm-plan.yaml`（由 `scripts/repo-knowledge-router/src/pm-sync-dir.mjs` 解析）
